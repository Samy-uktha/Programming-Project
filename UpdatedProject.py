...
# defining function for login page with username and password labels and entry boxes
    def create_login_page(self):
        ...
        self.username_label = tk.Label(
            self.master, text="Username:", font="Arial 20", background="salmon"
        )
        self.username_label.place(x=600, y=100)

        self.username_entry = tk.Entry(
            self.master, font="Arial 20", background="pink"
        )
        self.username_entry.place(x=530, y=150)


        self.password_label = tk.Label(
            self.master, text="Password:", font="Arial 20", background="salmon"
        )
        self.password_label.place(x=600, y=250)

        self.password_entry = tk.Entry(
            self.master, show="*", font="Arial 20", background="pink"
        )
        self.password_entry.place(x=530, y=300)

#creating a login button which checks if username is registered through login function
...
# creating a signup button which directs to signup window
        self.signup_button = tk.Button(
            ...
            command=self.create_signup_window,
        )
        self.signup_button.place(x=620, y=500)

# defining a function login to check if the given username and password matches with a predefined value or with data entered through signup function
    def login(self):
...
# creating a signup window with labels and entry boxes for a new username and password
    def create_signup_window(self):
        self.signup_window = tk.Toplevel(self.master)
        self.signup_window.title("Shopping Program")
        ...
        self.username_label = tk.Label(
            self.signup_window, text="Enter a Username", font="Arial 20", background="salmon"
        )
        self.username_label.place(x=600, y=100)

        self.username_entry = tk.Entry(
            self.signup_window, font="Arial 20", background="pink"
        )
        self.username_entry.place(x=530, y=150)


        self.password_label = tk.Label(
            self.signup_window, text="Enter a Password", font="Arial 20", background="salmon"
        )
        self.password_label.place(x=600, y=250)
...
# signup button which directs to signup function
        self.signup_button = tk.Button(
            self.signup_window,
            text="Sign up",
            font="Arial 20",
...
# function to add new username and password to the dictionary users
    def signup(self):
        username = self.username_entry.get()
        password = self.password_entry.get()
...
# adding a button for each section which opens the corresponding subsection window for that section
        for section in self.sections:
            section_button = tk.Button(
                ...
                text=section["name"],
                font="Arial 20",
                command=lambda section=section: [
                    section_window.destroy(),
                    self.open_subsection_window(section),
                ],
            )
            section_button.pack(pady=40)

# logout button which directs back to login page
        logout_button = tk.Button(
            ...
            background="light blue",
            activebackground="blue",
            command=lambda: [section_window.destroy(), self.create_login_page()],
        )
        logout_button.pack(pady=70)
...
# creating buttons for each subsection in the section which directs to the item window corresponding to that subsection
        for subsection in section["subsections"]:
            subsection_button = tk.Button(
                subsection_window,
                background="salmon",
                text=subsection["name"],
                font="Arial 20",
                command=lambda subsection=subsection: [
                    subsection_window.destroy(),
                    self.open_item_window(subsection),
                ],
            )
            subsection_button.pack(pady=30)

# back button which directs back to section window
        back_button = tk.Button(
            ...
            background="light blue",
            activebackground="blue",
            command=lambda: [subsection_window.destroy(), self.open_section_window()],
        )
...
# creating a frame for each item in the subsection
# adding item button, remove button, quantity label, and price label to each item frame
...
# bill button which directs to bill window
        generate_bill_button = tk.Button(
            ...
            background="light green",
            activebackground="green",
            command=lambda: [item_window.destroy(), self.open_bill_window()],
        )
...
#back button which directs back to section window
        back_button = tk.Button(
            ...
            background="light blue",
            activebackground="blue",
            command=lambda: [item_window.destroy(), self.open_section_window()],
        )
        back_button.pack(pady=20)

# function to add item to the cart if item not present in cart and increment quantity if item already present in cart
...
# function to remove item from cart and decrement quantity
...
# function to open a bill window with a table showing items in the cart, quantity, and total price
    def open_bill_window(self):
        ...
        bill_label = tk.Label(
            bill_window, width=30, text="Bill", font="Arial 30", background="pink"
        )
        bill_label.pack(pady=30)

        bill_table = ttk.Treeview(
            bill_window,
            columns=("name", "price", "quantity", "total price"),
            show="headings",
        )
        ...
        total_cost = sum(
            item["price"] * item["quantity"] for item in self.shopping_cart
        )
        total_label = tk.Label(
            bill_window, text=f"Total Cost: ${total_cost}", font="Arial 20"
        )
        total_label.pack(pady=10)

# creating a frame for labels and entry boxes of payment details like card number, card name, and cvv
        ...
        tk.Label(
            self.payment_frame, width=30, text="Card Number:", font="Arial 15"
        ).grid(row=1, column=0, sticky=tk.E)

        self.card_entry = tk.Entry(
            self.payment_frame, font="Arial 15"
        )
        self.card_entry.grid(row=1, column=1, padx=20, pady=10)

        tk.Label(
            self.payment_frame, width=30, text="Name on Card:", font="Arial 15"
        ...
        tk.Label(
            self.payment_frame, width=30, text="CVV:", font="Arial 15"
        ...
        back_button = tk.Button(
            ...
            background="light blue",
            activebackground="blue",
            command=lambda: [bill_window.destroy(), self.open_section_window()],
        )
        back_button.pack(pady=20)
...
# function to add users payment details
    ...
    def thank_you(self):
        ...
        thank_window.geometry("1920x1080")

        self.bg_image = tk.PhotoImage(file="thanks.png")
        self.bg_label = tk.Label(thank_window, image=self.bg_image)
        self.bg_label.place(x=0, y=0, relwidth=1, relheight=1)

        thank_label = tk.Label(
            thank_window, width=30, text="Thank you for shopping with us!", font="Cursive 30"
        )
        thank_label.pack(pady=30)
